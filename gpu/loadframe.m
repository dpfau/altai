...
data = zeros(1024,2048,41,'uint16');
for i = 1:41
    fid = fopen(sprintf('%s/dff_aligned_T%d_slice%d',dataPath,t,i),'r');
    slice = fread(fid,'uint16');
    fclose(fid);
    slice = reshape(slice,1472,2048);
    data(:,:,i) = slice(225:1248,:,:); % The animal is entirely within these bounds, so we can just throw out about 1/3rd of the data.
