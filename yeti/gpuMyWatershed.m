function watershed = gpuMyWatershed(I,marker)

watershed = gpuArray.zeros(size(I),'uint16');
idx = find(marker);
for id = 1:length(idx)
    watershed(idx(id)) = id;
end
...
while npix > 0
    ...
    boundary = gpuArray.zeros(size(watershed),'uint16');
    overlap = gpuArray.false(size(watershed));
    overlap_different = gpuArray.false(size(watershed));
    for i = -1:1
        for j = -1:1
            switch ndims(I)
                case 2
